/**
 * Unit tests for deleteLibraryItem — exercises the Prisma P2025 TOCTOU catch
...
import { deleteLibraryItem } from "./delete-library-item.server";

const mockedFindUnique = vi.mocked(prisma.libraryItem.findUnique);
const mockedDelete = vi.mocked(prisma.libraryItem.delete);
...
function makePrismaError(
  ...
  Object.defineProperties(err, {
    ...
    message: { value: `Prisma ${code}`, writable: false, enumerable: true },
    clientVersion: { value: "0.0.0", writable: false, enumerable: true },
  });
  return err;
}

describe("deleteLibraryItem — Prisma error mapping", () => {
  beforeEach(() => {
  ...
  describe("given prisma.libraryItem.delete throws P2025 (TOCTOU race)", () => {
    beforeEach(() => {
      mockedDelete.mockRejectedValue(makePrismaError("P2025", {}));
    });

    it("re-throws as NotFoundError", async () => {
      await expect(deleteLibraryItem(OWNER_ID, ITEM_ID)).rejects.toBeInstanceOf(
        NotFoundError
      );
    });
  });

  describe("given prisma.libraryItem.delete throws a non-P2025 Prisma error", () => {
    beforeEach(() => {
      mockedDelete.mockRejectedValue(makePrismaError("P2003", {}));
    });

    it("re-throws the raw Prisma error without wrapping", async () => {
      await expect(deleteLibraryItem(OWNER_ID, ITEM_ID)).rejects.toMatchObject({
        code: "P2003",
      });
    });
  });
