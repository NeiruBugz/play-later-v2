...
- **As a** library user, **I want** filters in a dedicated, persistent panel, **so that** I can see and change them without competing with my games for vertical space.
  - **Acceptance Criteria:**
    - [ ] On viewports ≥ 1280px, a left sidebar (`≥ 240px` wide) is visible on the Library page and contains: status filters with counts (2.5), platform filter, sort control.
    - [ ] On viewports between 768px and 1279px, the sidebar collapses to a narrow icon-only rail. A toggle expands it back to the full panel.
    - [ ] On viewports ≤ 640px, the sidebar is not used; mobile rules in 2.2 apply.
    - [ ] The search input is rendered above (or inline at the top of) the games grid, not inside the sidebar.
    - [ ] Sidebar collapsed/expanded state persists across page navigations within the same browser tab/session and resets on full reload.
    - [ ] Vertical space consumed by filter controls above the first row of game cards is reduced compared to the current inline layout.
...
- **As a** mobile user, **I want to** see every status filter without horizontal scrolling, **so that** I do not miss filters like "Playing" or "Played".
  - **Acceptance Criteria:**
    - [ ] On viewports ≤ 640px, status filters render as a segmented control with five segments (Wishlist, Shelf, Up Next, Playing, Played) using labels only — no count badges in the segments themselves.
    - [ ] If the five labels do not fit the viewport without truncation, the segmented control becomes horizontally scrollable with a visible scroll affordance (snap points, fading edge, or scroll indicator) — *not* the current invisible `overflow-x: auto`.
    - [ ] All segments are tap-targets at least 44px tall.
    - [ ] A "Filters" button on the Library page header opens a bottom sheet containing the platform filter, the sort control, and a list of statuses with counts (2.5).
    - [ ] Removing the existing chip strip does not introduce horizontal page scroll.
...
- **As a** dashboard user with multiple in-progress games, **I want** one obvious action per game and a sensible ordering, **so that** I am not paralyzed by choice.
  - **Acceptance Criteria:**
    - [ ] Each game in the Quick Log hero exposes exactly one primary CTA: **Log Session**, styled as a button.
    - [ ] **Reflect** is rendered as a secondary text link beneath the Log Session button — not styled as a peer button.
    - [ ] No other actions on the same card are styled as peer buttons to Log Session.
    - [ ] Games shown are sorted by most-recent-activity first, using `LibraryItem.startedAt` and/or the latest journal-entry `createdAt`. Tie-breaker: `LibraryItem.updatedAt` descending.
    - [ ] When no games are in `PLAYING` status, the existing empty-state CTA in the hero is preserved.
...
- **As a** library scanner, **I want** game status conveyed with both color and text, **so that** I can identify status at a glance and the UI passes WCAG 1.4.1.
  - **Acceptance Criteria:**
    - [ ] Each library card overlays a pill badge in the top-left of the cover containing the localized status text (e.g., "Playing", "Played", "Wishlist", "Up Next", "Shelf").
    - [ ] The badge uses a status color **and** text — never color alone.
    - [ ] When the user has applied a single-status filter and the grid is therefore homogeneous, the badge is hidden (current behavior preserved).
    - [ ] The redundant status text label below the cover is removed; that slot is reused for metadata (see 2.7).
    - [ ] Badge text contrast against any cover image area passes WCAG AA (4.5:1) — a translucent dark or light backdrop is acceptable.
...
- **As a** library user, **I want** to see how many games match each status under my current other-filter context, **so that** I do not click into empty filters and I get progress feedback as my backlog shrinks.
  - **Acceptance Criteria:**
    - [ ] Every status filter row in the desktop sidebar shows a count next to the label (e.g., "Playing 4").
    - [ ] On mobile, the same counts appear next to status entries in the Filters bottom sheet (2.2). The mobile segmented control itself does not show counts.
    - [ ] Counts reflect the games the user would see if that status were applied alongside the currently-active platform/search filters.
    - [ ] Filters with a zero count under the current other-filter context are visually dimmed but remain clickable.
    - [ ] Counts are returned by the same server query that loads the page's library data — no separate per-chip request.
    - [ ] When a user changes a game's status locally, counts update optimistically and reconcile on server response.
...
- **As a** user with 100+ games, **I want** search to be the most prominent control on the Library page, **so that** I can find a game without scrolling.
  - **Acceptance Criteria:**
    - [ ] On all viewports, the search input occupies a row above the games grid, full width of the grid column, with a minimum height of 44px.
    - [ ] The input shows a visible focus state (border glow or outline ring) on focus.
    - [ ] A keyboard shortcut hint (`/`) is shown inside the input, right-aligned.
    - [ ] Pressing `/` from anywhere on the Library page (when no input is already focused) focuses the input. The shortcut focuses the existing input only — there is no command palette overlay opened by `/`.
    - [ ] `⌘K` (or `Ctrl K`) is **not** bound by the library hero search. It is owned by the global command palette per 2.11.
    - [ ] The input is not sticky on scroll.
    - [ ] Existing placeholder text ("Filter library…") is preserved.
...
- **As a** patient gamer, **I want** each card to show one piece of metadata appropriate to the game's status, **so that** I get useful context without opening the detail page.
  - **Acceptance Criteria:**
    - [ ] The space previously used by the redundant status text label (now moved to the cover badge per 2.4) shows status-appropriate metadata:
    ...
    - [ ] No new database fields are required — implementation must use existing `LibraryItem` fields: `status`, `rating`, `hasBeenPlayed`, `createdAt`, `updatedAt`, `startedAt`, `actualPlaytime`, `acquisitionType`, `platformId`.
    - [ ] When `actualPlaytime` is null or zero, the UI must not display "0h played" — it falls back per the rules above.
    - [ ] Existing rating-stars display is preserved unchanged.
...
- **As a** new user with an empty library, **I want** clear paths to add my first games, **so that** I reach first value without trial and error.
  - **Acceptance Criteria:**
    - [ ] When a signed-in user's library is empty, the Library page (and the Dashboard's library widget if applicable) renders an embedded hero section with the heading **"Start Your Gaming Journey"** and two primary CTAs:
      1. **Import from Steam** — links to the existing Steam import flow.
      2. **Search for Games** — focuses the IGDB search.
    - [ ] The hero includes a short description of what SavePoint does (one or two sentences) and at minimum a static illustration or textual preview of what a populated library looks like.
    - [ ] The hero is shown only when the user has zero `LibraryItem` rows. As soon as they add one game, the hero is replaced by the standard library/dashboard view.
    - [ ] The existing "Find a game to start" empty-state button is removed or absorbed into the new hero so users see one consistent empty state, not two.
    - [ ] The hero is embedded in the page — no modal, no coachmark/tour overlay.
...
- **As a** user changing a filter, sort, or search term, **I want** the grid to update instantly and the control to acknowledge my input, **so that** the app feels responsive and I do not double-click.
  - **Acceptance Criteria:**
    - [ ] On a filter chip click, sort change, or debounced search keystroke, the games grid updates client-side immediately (optimistic) using already-fetched library data.
    - [ ] Simultaneously, the triggering control enters a disabled state and shows a small spinner (in-chip / on the sort control / in the search input) until the server response arrives.
    - [ ] While a fetch is in flight, filter chips and the sort control are not clickable; the search input remains typeable but its outgoing requests are debounced to a single in-flight request at a time.
    - [ ] When the server response arrives, the grid reconciles with authoritative data; if the optimistic prediction was wrong, the diff is applied without a hard re-render flash.
    - [ ] Input-handling uses `useTransition` (or equivalent) so the UI thread is never blocked during state updates.
    - [ ] The existing `LibraryGridSkeleton` is still used for the initial page load and full route transitions — it is not used for in-page filter/sort/search changes (those are optimistic instead).
...
- **As a** library user on any display, **I want** cards to stay at a readable width as my viewport changes, **so that** metadata and CTAs do not become unreadable on large screens or get cramped on small ones.
  - **Acceptance Criteria:**
    - [ ] Desktop (≥1024px): grid uses `repeat(auto-fill, minmax(180px, 220px))` with a 16px gap. Cards never render narrower than 180px.
    - [ ] Tablet (768–1023px): grid uses `repeat(auto-fill, minmax(160px, 200px))` with a 14px gap.
    - [ ] Mobile (≤640px): the grid is replaced by the list view defined in 2.10.5 — not a single-column grid.
    - [ ] The mid range (641–767px) renders as a grid with `minmax(150px, 1fr)` to bridge phone-landscape to tablet without a layout cliff.
    - [ ] Card cover preserves a 3:4 aspect ratio at all widths.
    - [ ] The previous fixed column system (`grid-cols-3 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10 2xl:grid-cols-12`) is removed.
...
- **As a** patient gamer, **I want** each card to surface platform, recency, rating, and a primary action without hovering, **so that** I can triage my backlog by reading, not by mousing over each tile.
  - **Acceptance Criteria:**
    - [ ] Each card renders, top-to-bottom: cover (with status badge per 2.4 and context menu per 2.10.4) → title (14px, 2-line clamp) → metadata row (2.10.3) → rating stars (2.10.6) → primary CTA button (2.10.4).
    - [ ] No card content depends on hover to be readable or actionable.
    - [ ] Title uses 14px / weight 600; metadata row uses 11px / muted color; CTA uses 13px / weight 600 inside a 36–40px tall button.
    - [ ] Spacing between stack elements is 6–8px; the CTA gets 8px top margin.
...
- **As a** library scanner, **I want** every card to show platform and a relevant date so I can identify and recall games at a glance.
  - **Acceptance Criteria:**
    - [ ] Metadata row format: `<platform badge> • <contextual date>` rendered inline.
    - [ ] Platform badge uses the existing `platformId` lookup; badge style matches the muted secondary-badge token already in shadcn/ui usage. If `platformId` is null, the badge and the bullet separator are omitted (date renders alone).
    - [ ] Contextual date is a relative string derived from `LibraryItem` fields by status:
    ...
    - [ ] Relative formatting matches the project's existing date helper (e.g., `3d ago`, `2w ago`, `5mo ago`).
    - [ ] No new database fields are introduced. `actualPlaytime` is not displayed in the metadata row in this revision; if/when session logging populates it, a follow-up spec may re-introduce it.
    - [ ] "0h played" or any zero-valued metadata never renders.
...
- **As a** library user, **I want** one obvious action per card and a non-hidden way to reach secondary actions, **so that** the card is usable on touch and keyboard, not just on hover.
  - **Acceptance Criteria:**
    - [ ] Each card renders exactly one always-visible primary CTA button under the rating row, full card width.
    - [ ] CTA label is status-aware:
    ...
    - [ ] CTA click does not navigate; clicking the cover or the title still navigates to the game detail page.
    - [ ] A context menu trigger (⋮) renders absolutely positioned in the cover's top-right (28×28px hit area, semi-transparent dark backdrop, ≥3:1 contrast against any cover image). It is always visible — not hover-revealed.
    - [ ] Context menu items: **View Journal Entries**, **Edit Library Details**, **Change Status →** (submenu of all five statuses), **Remove from Library**.
    - [ ] The existing hover-overlay quick-action UI on cards is removed; its actions are migrated into the primary CTA + context menu per the matrix above.
    - [ ] Both the CTA and the context-menu trigger preserve event propagation so clicking them does not also trigger card navigation.
    - [ ] All interactive elements meet 44×44px touch-target minimums on mobile (2.10.5).
...
- **As a** mobile user, **I want** a scannable list of my games rather than a dense grid of thumbnails, **so that** titles and CTAs are legible without zooming.
  - **Acceptance Criteria:**
    - [ ] On viewports ≤640px, the library renders as a vertical list. Each row uses `flex-direction: row`, 12px gap, 12px padding, and a divider or 8px vertical spacing between rows.
    - [ ] Row layout: cover thumbnail (60×80px, fixed, `flex-shrink: 0`) on the left; content column (`flex: 1`, `min-width: 0`) on the right.
    - [ ] Content column shows: title (15px, 1-line clamp), metadata row per 2.10.3 (12px), rating stars per 2.10.6, primary CTA per 2.10.4 (full row width, ≥44px tall).
    - [ ] Status badge per 2.4 still overlays the thumbnail's top-left.
    - [ ] Context menu (⋮) is anchored to the row's top-right, with the same 44×44px touch target.
    - [ ] The mid range (641–767px) does not use the list view — it uses the grid per 2.10.1.
    - [ ] Switching between list and grid is purely viewport-driven; no user toggle is added in this spec.
...
- **As a** user finishing or sampling a game, **I want** to set or change a rating from the card without opening the detail page, **so that** quick triage is possible during library scans.
  - **Acceptance Criteria:**
    - [ ] Each card's rating row is interactive: clicking a star sets the rating to that value (1–5 on a card UI, mapped to the 1–10 `rating` column by `value × 2`, preserving compatibility with spec 011).
    - [ ] Clicking the currently selected star clears the rating (sets `rating = null`).
    - [ ] Updates apply optimistically and reconcile with the server response; on failure, the previous value is restored and the existing error toast is shown.
    - [ ] Clicking a star does not trigger card navigation.
    - [ ] Keyboard support: focusable star group, Left/Right arrows change rating, Enter commits, Escape cancels and restores prior value.

#### 2.10.7 Migration & Compatibility

- **Acceptance Criteria:**
  - [ ] No `LibraryItem` schema changes are required. All fields used (`status`, `rating`, `hasBeenPlayed`, `createdAt`, `updatedAt`, `startedAt`, `platformId`) already exist.
  - [ ] The status badge defined in 2.4 remains the source of truth for status presentation; 2.10 does not add a redundant status text under the cover.
  - [ ] The single-status-filter dedupe behavior from 2.4 is preserved: when a single status filter is active, the cover badge hides but all other 2.10 elements (metadata row, CTA, context menu, rating, mobile list view) render unchanged.
  - [ ] The existing `LibraryGridSkeleton` is updated to match the new card dimensions and the mobile list layout.
  - [ ] Visual styling reuses existing design tokens (colors via the project's CSS variables, shadcn/ui `Button`, `Badge`, `DropdownMenu` primitives, Tailwind utilities). No new design system is introduced.
...
- **As a** user browsing IGDB search results, **I want** to add a game to my library in one click without leaving the search surface, **so that** I can bulk-add games while exploring.
  - **Acceptance Criteria:**
    - [ ] Every game-search result card (the IGDB results surface used in onboarding/empty-library and in the global search experience) renders a 32×32 `+` button anchored to the card's top-right.
    - [ ] Clicking the `+` button calls a `quickAddToLibraryAction` server action with `{ gameId }`, which adds a `LibraryItem` for the current user using the smart defaults above.
    - [ ] If the game is already in the user's library, the button shows the success/check state and is disabled (no duplicate-create error surfaced to the user).
    - [ ] Optimistic UI: the button flips to a check state immediately on click; on error, the button restores and a destructive toast appears.
    - [ ] On success, a toast reads `Added "<game name>" to Up Next` with an **Undo** action visible for ~5 seconds (2.11.5).
    - [ ] Clicking the `+` button does not navigate to the detail page; clicking elsewhere on the card still navigates as today.
    - [ ] The button has an accessible label (`aria-label="Quick-add <game name> to library"`) and is keyboard-reachable.
...
- **As a** keyboard-driven user, **I want** to add a game from anywhere in the app via the command palette, **so that** I do not have to navigate to a search page first.
  - **Acceptance Criteria:**
    - [ ] `⌘K` (macOS) / `Ctrl K` (other platforms) opens the existing global command palette from any authenticated route.
    - [ ] The palette surfaces a Quick Add mode (either always-on or via an "Add game to library" action). Typing ≥ 2 characters runs an IGDB search and shows up to 8 result rows.
    - [ ] Each result row shows: game title, platform badge, year/category if available, and a right-aligned hint reading `Add to Up Next`.
    - [ ] Selecting a row (Enter or click) calls the same `quickAddToLibraryAction` and closes the palette.
    - [ ] Toast feedback and Undo behave per 2.11.5.
    - [ ] `Esc` closes the palette without adding.
    - [ ] The command palette retains any pre-existing actions (Slice 6 and earlier) — Quick Add is added, not replacing.
...
- **As a** user on any authenticated page, **I want** a globally-visible Add Game entry point, **so that** I do not have to know which page to start from.
  - **Acceptance Criteria:**
    - [ ] The header (`widgets/header/ui/header.tsx`) shows an **Add Game** button on viewports ≥ 768px. On mobile (≤ 640px) the button collapses to an icon-only `+` with `aria-label="Add Game"`.
    - [ ] Clicking the button opens the global command palette in Quick Add mode (2.11.2). It does **not** open a separate modal.
    - [ ] The button is rendered next to the existing `⌘K` palette trigger if one exists, or replaces it (audit during implementation; only one entry point should exist).
...
- **As a** user who wants to set status / platform / acquisition explicitly at add-time, **I want** the existing full add-to-library modal on the game detail page to still work.
  - **Acceptance Criteria:**
    - [ ] The existing "Add to Library" modal on the game detail page is unchanged in this spec.
    - [ ] No code path in this spec routes the detail-page Add flow through `quickAddToLibraryAction`; it continues to use the existing form-based action.
...
- **As a** user who clicks Quick Add by mistake, **I want** a quick way to undo, **so that** misclicks do not pollute my library.
  - **Acceptance Criteria:**
    - [ ] After a successful Quick Add (any surface), a toast renders for ~5 seconds containing: success icon, text `Added "<game name>" to Up Next`, and a textual **Undo** button.
    - [ ] Clicking **Undo** within the toast lifetime calls a `removeLibraryItemAction` (existing remove flow) for the just-added item; the toast is replaced with a brief `Removed` confirmation.
    - [ ] If the toast lifetime expires without action, the item remains in the library; no further confirmation is required.
    - [ ] Toast and Undo behavior is identical across all three Quick Add surfaces (inline `+`, command palette, navbar trigger that delegates to command palette).
    - [ ] If multiple Quick Adds happen in rapid succession, each emits its own toast (no merging), and Undo on a given toast only removes the specific item it refers to.
...
- **As a** user adding games whose primary platform should be guessed correctly, **I want** the auto-detected platform to match my collection patterns when possible.
  - **Acceptance Criteria:**
    - [ ] Auto-detect resolves a `platformId` from the IGDB game's platform list using the first platform that exists in our local `Platform` table, in the order returned by IGDB.
    - [ ] If no IGDB platform matches a local `Platform`, `platformId` is set to `null`. The add still succeeds.
    - [ ] If the user has previously chosen a platform for a different game in the same Steam-import / browsing session, that preference is **not** used by this spec — auto-detect is stateless. (A "preferred platform" preference is out of scope.)
    - [ ] The auto-detect result is editable per 2.12 after the item is added.
...
- **As a** user who notices the platform on a library item is wrong (especially after Quick Add auto-detect), **I want** to change it without removing and re-adding the game, **so that** my library reflects the right platform.
  - **Acceptance Criteria:**
    - [ ] The existing Edit Library Details dialog/form (reached from the card's `⋮` menu per 2.10.4 and from the detail page) exposes a `platform` field that can be set, changed, or cleared.
    - [ ] Submitting the form persists `platformId` (or `null`) on the `LibraryItem` row.
    - [ ] The change is reflected immediately on the card metadata row (2.10.3) and on any platform-derived filter/count (2.5) on next render.
    - [ ] No new database fields are introduced; `LibraryItem.platformId` is already nullable and writable in schema.
    - [ ] If the underlying server action / use-case currently rejects `platformId` updates, that restriction must be lifted as part of this section.
    - [ ] Existing platform-related validation (must reference an existing `Platform` row when non-null) is preserved.
