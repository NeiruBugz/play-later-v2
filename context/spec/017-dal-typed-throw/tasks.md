...
- [x] **Pre-Reaper grep:** `rg "ServiceResult|RepositoryResult|serviceSuccess|serviceError|repositorySuccess|repositoryError|handleServiceError|BaseService|isSuccessResult|isErrorResult|RepositoryErrorCode|ServiceErrorCode" savepoint-app/{app,features,data-access-layer,shared}` should return matches **only** in `data-access-layer/{repository/errors.ts,services/types.ts,services/index.ts,repository/index.ts}` and tests being deleted in this slice. If anywhere else, that domain wasn't fully converted — file follow-up. **[Agent: inline]** _(audit clean — only the 4 expected sources, plus `test/helpers/service-test-helpers.ts` cleaned in this slice)_
- [x] Delete `data-access-layer/repository/errors.ts`. **[Agent: inline]** _(plus `data-access-layer/repository/types.ts` which only re-exported `NotFoundError`/`DuplicateError` from the deleted file)_
- [x] Edit `data-access-layer/services/types.ts`: remove `ServiceResult`, `ServiceErrorCode`, `serviceSuccess`, `serviceError`, `handleServiceError`, `isSuccessResult`, `isErrorResult`, `ExtractServiceData`. Keep `PaginatedResult`, `PaginationInput`, `CursorPaginatedResult`, `BaseServiceInput`. **[Agent: inline]**
- [x] Delete `data-access-layer/services/base-service.ts` (if it exists as its own file). **[Agent: inline]** _(no-op — never existed as a separate file; was an inline `extends BaseService` reference inside services that were converted slice-by-slice)_
- [x] Update `data-access-layer/repository/index.ts` and `data-access-layer/services/index.ts`: drop re-exports of deleted symbols. **[Agent: inline]**
- [x] Update `data-access-layer/CLAUDE.md`: rewrite "Result Pattern" section as "Error Model" (typed throws + edge catches + link to ADR `context/decisions/DAL_TYPED_THROW.md`). Delete the two trip-wires "Result type shapes are NOT interchangeable" and "Some repository functions return plain rows, not RepositoryResult". Keep the other trip-wires. **[Agent: inline]** _(also added a class-identity trip-wire — the silent-bug surface that bit Slices 3 and 10)_
- [x] Update `data-access-layer/repository/CLAUDE.md`: rewrite "Not-Found Handling" section to describe the type-driven contract (`findX → T | null` for may-miss, `requireX → T` throws for must-exist, list/filter → `T[]`, count → primitive, modification on missing entity → throws `NotFoundError`). **[Agent: inline]** _(also documented Prisma error translation table — P2025 → NotFoundError, P2002 → ConflictError, others rethrow)_
- [x] Update `data-access-layer/services/CLAUDE.md` and `data-access-layer/services/README.md`: replace "Result-based errors" / "extends BaseService" guidance with "throw typed errors" / "trust typed input — no re-validation." **[Agent: inline]** _(README reduced from 626 LOC to a 47-LOC pointer + at-a-glance examples; the per-service method documentation was a brittle source of drift and is replaced by code as source-of-truth)_
- [x] **Bonus:** update `features/CLAUDE.md` trip-wires #1 and #2 — replace the old `RepositoryResult.ok` vs `HandlerResult.success` confusion + raw-row inconsistency entries with the new typed-throw model summary + class-identity trip-wire. **[Agent: inline]**
- [x] **Bonus:** clean up `test/helpers/service-test-helpers.ts` — delete the orphan `expectServiceSuccess`/`expectServiceError`/`createSuccessResult`/`createErrorResult`/`isServiceSuccess`/`isServiceError` helpers and their `ServiceResult` import. Keep `createMockDate`, `buildMockUser`, `buildMockGame`, `buildMockLibraryItem`, `buildMockJournalEntry`, `buildMockReview`, `waitFor`, `createPartialMock`, `createRejectedPromise`, `createResolvedPromise`. **[Agent: inline]**
- [x] **Bonus:** drop `DuplicateError` from `data-access-layer/services/auth/auth-service.unit.test.ts` — replaced with plain `new Error("Unique constraint failed...")` (auth service detects unique-constraint via `error.message.includes("Unique constraint")` so a plain Error works); rename misnamed test in `follow-repository.integration.test.ts:38` from "DuplicateError" to "ConflictError". **[Agent: inline]**
- [x] Bump `Status:` from `Draft` → `In Review` in `context/spec/017-dal-typed-throw/{functional-spec.md,technical-considerations.md}`. **[Agent: inline]**
- [x] **Verify:** `pnpm --filter savepoint ci:check` green. `pnpm test --project=integration` green. Final canonical grep `rg "ServiceResult|RepositoryResult|serviceSuccess|serviceError|repositorySuccess|repositoryError|handleServiceError|BaseService|isSuccessResult|isErrorResult|RepositoryErrorCode|ServiceErrorCode" savepoint-app/{app,features,data-access-layer,shared} --glob '!*.test.ts'` returns **zero** hits. Smoke-boot the app, sign in, browse a few pages — no console errors. **[Agent: inline]** _(typecheck/lint/format pass; utilities 124, components 812, backend 790, integration 415; final grep matches only `BaseServiceInput` (a pagination-input type that survives per spec — false-positive substring match on `BaseService`); manual UI smoke deferred to human pre-merge)_
