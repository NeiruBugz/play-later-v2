...
function TopBar({ title, showLogo = false, onBack = false, trailing }) {
  return (
    <header style={{
      ...
      {onBack ? (
        <button aria-label="Back" style={iconBtn}><Icon name="ChevronLeft" size={22} /></button>
      ) : showLogo ? (
    ...
    <nav style={{
      ...
      {center && (
        <div style={{ flex: "0 0 auto", display: "flex", alignItems: "flex-start", justifyContent: "center", width: 64 }}>
          <button aria-label="Log a session" style={{
...
function Sheet({ title, children, footer, grabber = true, height = "auto", dark, keyboard }) {
  return (
      ...
      <div style={{
        ...
        {title && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 18px 12px" }}>
            ...
            <button aria-label="Close" style={{ ...iconBtn, width: 34, height: 34, color: "var(--muted-foreground)" }}><Icon name="X" size={20} /></button>
          </div>
        )}
...
Object.assign(window, {
  ...
  TopBar, BottomNav, Segmented, SectionHead, Sheet, Field, dens, iconBtn, CriticRing,
});
