# CLAUDE.md - Manage Library Item Feature

This file provides comprehensive documentation for the manage-library-item feature module, following the established patterns from the main CLAUDE.md file.

## Feature Overview

The Manage Library Item feature provides comprehensive CRUD operations for users to manage their personal gaming library. It enables users to add games to their collection, track their gaming journey through different exploration statuses, manage multi-platform ownership, and maintain detailed gaming timelines with start and completion dates.

### Key Capabilities

- **Create**: Add games to library with platform and status information
- **Read**: Fetch user's library items for specific games
- **Update**: Modify status, platform, dates, and other metadata
- **Delete**: Remove items from library with confirmation
- **Quick Actions**: One-click status updates (Mark as Experienced, Start Exploring, etc.)
...
├── create-library-item/          # Creation functionality
│   ├── components/
...
├── edit-library-item/            # Update operations
│   ├── components/
...
│   ├── hooks/
│   │   ├── update-library-action.ts
│   │   └── use-matching-library-item.ts
│   ├── lib/
│   │   └── validation.ts
│   ├── server-actions/
│   │   ├── action.ts
...
│   │   └── schema.ts
│   └── index.ts
├── delete-library-item/          # Deletion functionality
│   ├── components/
│   │   ├── delete-library-item.tsx
│   │   └── index.ts
│   ├── server-actions/
│   │   ├── action.ts
│   │   └── action.server-action.test.ts
...
1. **Create Flow**:
   ...
   ```
   CreateLibraryItemForm → createLibraryItem action → createLibraryItemCommand repository → Database
   ```

2. **Update Flow**:

   ```
   Quick Action Button → updateLibraryItemAction → updateLibraryItem repository → Database
   Edit Modal Form → editLibraryItem action → updateLibraryItem repository → Database
   ```

3. **Delete Flow**:
   ```
   DeleteLibraryItem Component → deleteLibraryItemAction → deleteLibraryItem repository → Database
   ```
...
**Key Component**: `CreateLibraryItemForm` (`create-library-item/components/create-library-item-form.tsx`)
...
**Server Action**: `createLibraryItem` (`create-library-item/server-actions/action.ts`)
...
1. **EditGameEntryModal** (`edit-game-entry-modal.tsx`)
   ...
   - Lines 48-54: Dynamic tab generation based on existing library items
   - Lines 57-77: Tab content rendering with form components

2. **GameEntryForm** (`game-entry-form.tsx`)

   - Lines 37-177: Comprehensive form for editing all library item properties
...
3. **Quick Action Buttons**:
   ...
   - **StartPlayingActionButton** (lines 25-75): One-click status change to "CURRENTLY_EXPLORING"
   - **MoveToBacklogActionButton**: Similar pattern for "CURIOUS_ABOUT" status

**Hooks**:

- **useMatchingLibraryItem** (`hooks/use-matching-library-item.ts`): Finds library items by status with URL param support
- **updateLibraryActionAction** (`hooks/update-library-action.ts`): Server action for quick status updates

**Server Actions**:

- **editLibraryItem** (`server-actions/action.ts`): Full edit with comprehensive validation
- **updateLibraryItemAction** (`hooks/update-library-action.ts`): Quick status-only updates
- **getLibraryItems** (`server-actions/get-library-items.ts`): Fetch user's items for a game

### Delete Sub-Feature

**Key Component**: `DeleteLibraryItem` (`delete-library-item/components/delete-library-item.tsx`)
...
**Zod Form Data Patterns** (`edit-library-item/server-actions/schema.ts`):

- Lines 5-11: LibraryItemStatus enum validation
...
**LibraryItem Structure** (from PRD.md, lines 191-203):
...
- `createLibraryItem`: Create new library entries
- `updateLibraryItem`: Update existing entries
- `deleteLibraryItem`: Remove entries with ownership validation
- `getManyLibraryItems`: Fetch user's items for a game
...
**Server Action Testing Pattern** (`create-library-item/server-actions/action.server-action.test.ts`):
...
2. **Data Fetching**:
   ...
   - Selective fetching with `getManyLibraryItems`
...
- Machine learning recommendations for next games to explore
- Social features (compare libraries, friend recommendations)
...
import { CreateLibraryItemForm } from "@/features/manage-library-item/create-library-item";

<CreateLibraryItemForm gameId="game-123" />

// Editing entries
import { EditGameEntryModal } from "@/features/manage-library-item/edit-library-item";

<EditGameEntryModal libraryItems={userLibraryItems} />

// Quick actions
import { CompleteActionButton, StartPlayingActionButton } from "@/features/manage-library-item/edit-library-item";

<CompleteActionButton libraryItems={items} />
<StartPlayingActionButton game={gameData} libraryItems={items} />
```
...
```typescript
// Quick status update
import { updateLibraryItemAction } from "@/features/manage-library-item/edit-library-item/hooks/update-library-action";
// Full edit
...
formData.append("status", "CURRENTLY_EXPLORING");
