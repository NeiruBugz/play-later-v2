...
1. **DAL services and repositories throw typed errors; they do not return `Result` wrappers.** Services and repositories return raw data on success and throw a class from `@/shared/lib/errors` (or a co-located `services/<domain>/errors.ts` for domain-specific subclasses like `IgdbRateLimitError`, `SteamProfilePrivateError`) on failure. The server-action wrapper at `shared/lib/server-action/create-server-action.ts` catches and serializes throws into `ActionResult.error`. RSC pages use try-then-render (data fetched in `try`, JSX outside) to satisfy the `react-hooks/error-boundaries` lint rule. Handlers use `mapErrorToHandlerResult` plus optional `instanceof` overrides for status codes. See `data-access-layer/CLAUDE.md` "Error Model" and `context/decisions/DAL_TYPED_THROW.md`.
2. **Class identity matters for typed errors.** Always import error classes from `@/shared/lib/errors` (or the co-located `services/<domain>/errors.ts`). Importing the same class name from any other path produces a different prototype chain — `instanceof` silently returns `false`, `mapErrorToHandlerResult` falls through to 500, and `notFound()` redirects in RSC pages don't fire.
